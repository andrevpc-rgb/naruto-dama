...
app.post('/api/auth/register', requireDb, async (req, res) => {
  ...
  try {
    ...
    const user = await User.create({ username: usernameNorm, email: emailNorm, passwordHash });
...
app.get('/api/ranking', requireDb, async (req, res) => {
  ...
  try {
    const users = await User.find().sort({ seasonPoints: -1, wins: -1 }).limit(limit)
      .select('username seasonPoints wins losses').lean();
    res.json({
      ranking: users.map((u, i) => ({
        position: i + 1, username: u.username, seasonPoints: u.seasonPoints, wins: u.wins, losses: u.losses
      }))
    });
  } catch (err) {
...
function startPvpMatch(entryA, entryB) {
  ...
  entryA.socket.emit('match_found', { isBot: false, role: 'host', code, opponentNick: nickB });
  entryB.socket.emit('match_found', { isBot: false, role: 'guest', code, opponentNick: nickA });
}

// Ninguém apareceu em 15s — avisa o cliente pra ativar o bot LOCALMENTE (ver comentário no topo do
// arquivo). O servidor não simula peça nenhuma aqui, só manda o apelido falso.
function startBotMatch(entry) {
  entry.socket.emit('match_found', { isBot: true, opponentNick: randomBotNickname() });
}
